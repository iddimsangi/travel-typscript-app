...
const Main = ({renderedItems, packingChecker, deleteItemHandler, clearItemsHandler,sortedItemsKeyWord, setSortedItemsKeyWord}:PropsType) => {
  return (
    <div className="bg-yellow-900 flex flex-col py-6 px-4 ">
          <div className=" grid grid-cols-3 justify-between gap-2 py-2">          
           {
            renderedItems.map((item) => <Item key={item.id} item={item} deleteItemHandler={deleteItemHandler} packingChecker={packingChecker}/>)
           }
          </div>
